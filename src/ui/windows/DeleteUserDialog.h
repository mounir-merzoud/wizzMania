...
public:
    explicit DeleteUserDialog(QWidget* parent = nullptr) : QDialog(parent) {
        setWindowTitle("Supprimer un utilisateur");
        setModal(true);
        ...
        auto* title = new QLabel("Supprimer un utilisateur", this);
        ...
        connect(m_delete, &QPushButton::clicked, this, &DeleteUserDialog::onDeleteClicked);
...
private slots:
    void onDeleteClicked() {
        ...
        if (!item) {
            showError("Sélectionne un utilisateur.");
            return;
        }

        const QString userId = item->data(Qt::UserRole).toString();
        if (userId.isEmpty()) {
            showError("user_id invalide");
            return;
        }
        ...
        const auto answer = QMessageBox::warning(
            ...
            QStringLiteral("Supprimer cet utilisateur ?"),
        ...
        auto future = AuthService::instance().deleteUserAsAdmin(userId);
        ...
        connect(watcher, &QFutureWatcher<bool>::finished, this, [this, watcher]() {
            ...
            if (!ok) {
                const QString msg = AuthService::instance().lastError().isEmpty()
                                        ? QStringLiteral("Suppression utilisateur échouée")
...
private:
    void populateFromContacts() {
        ...
        for (const auto& c : contacts) {
            ...
            const QString label = c.email().isEmpty()
                                      ? QStringLiteral("%1 (%2)").arg(c.name(), id)
                                      : QStringLiteral("%1 (%2) — %3").arg(c.name(), c.email(), c.status());
        ...
        if (count == 0) {
            auto* item = new QListWidgetItem("Aucun utilisateur disponible (charge /users)", m_list);
